...
public class Customer_Support {
	...
	Customer_Handler custObj;
	
	public Customer_Support(String ID) {
		customerName = ID;
		newQuery = false;
		...
		b1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				...
				else if (cb1.getSelectedIndex()>0 && queryText.getText()!="") {
					response = responseText.getText();
		...
		cb1.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent arg0) {
				...
				if(cb1.getSelectedItem()!="" && arg0.getStateChange() == ItemEvent.SELECTED && !newQuery) {
					if ((String)cb1.getSelectedItem()!="No Active Ticket") {
						System.out.println("IM IN HERE BRO");
						initializeDetails();
					}
					else {
						System.out.println("IM IN HERE BRO+");
	...
	protected void initializeDetails() 
	{
		...
		query = custObj.getResponses(ticketID);
		setInfoLabels();
	...
	protected void newQuery() 
	{
		...
		ticketID = custObj.GenerateNewTicketID();
		response = "";
		cb1.removeAllItems();
		cb1.addItem("");
		cb1.addItem(ticketID);
		setInfoLabels();
		
		
	}
	
	protected boolean respondToAssistant() 
	{
		return custObj.respondToAssistant(ticketID, customerID, assistantID, response);
	}
	protected boolean askAssistant() 
	{
		return custObj.respondToAssistant(ticketID, customerID, assistantID, response);
	}
	...
	{
		customerID = custObj.getCustomerID(customerName);
		cb1.addItem("");
		String[] ticketIDs = custObj.getCustomerSupportIDs(customerID);
		for (int i=0; i<ticketIDs.length; i++) {
			cb1.addItem(ticketIDs[i]);
		}
		if (ticketIDs.length == 0) {
			cb1.addItem("No Active Ticket");
		}
	}
}
